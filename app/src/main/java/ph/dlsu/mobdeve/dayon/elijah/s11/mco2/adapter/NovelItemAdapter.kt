...
import ph.dlsu.mobdeve.dayon.elijah.s11.mco2.activities.EpisodeViewer
...
//                Toast.makeText(context, "${titleNovel[position]} ${date[position]} ${numEpisodes[position]} ", Toast.LENGTH_SHORT).show()
...
//                    putExtra("num_episodes", itemCount?.text)
                    if (position == 3){
                        itemView.setOnClickListener{
                            val intent = Intent(context, FrontEndNovel::class.java)
